...
fn array_reductions_match_expected_values() {
    ...
    assert_eq!(array.sum().unwrap(), 21);
    assert_eq!(array.prod().unwrap(), 720);
...
fn scalar_reductions_return_scalar_values() {
    ...
    assert_eq!(array.sum().unwrap(), 9);
    assert_eq!(array.prod().unwrap(), 9);
...
fn axis_reduction_empty_and_scalar_semantics_are_stable() {
    ...
    assert_eq!(empty_axis.sum_axis(0).unwrap_err(), AtlasNdError::EmptyReduction { op: "sum" });
    assert_eq!(empty_axis.prod_axis(0).unwrap_err(), AtlasNdError::EmptyReduction { op: "prod" });
...
fn empty_boundary_slices_preserve_axis_reduction_semantics() {
    ...
    assert_eq!(view.sum_axis(0).unwrap_err(), AtlasNdError::EmptyReduction { op: "sum" });
    assert_eq!(view.prod_axis(1).unwrap_err(), AtlasNdError::EmptyReduction { op: "prod" });
